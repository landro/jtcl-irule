...
public class Expression {
	...
	*/
	 ...
	 */
	private final ExprValue[] cachedExprValue;
	...
	public void evalSetResult(final Interp interp, final String string) throws TclException {
	 ...
	 */
	public boolean evalBoolean(final Interp interp, final String string) throws TclException {
		ExprValue value = ExprTopLevel(interp, string);
	 ...
	 * @return the value of the expression.
	 */
	private final ExprValue ExprTopLevel(final Interp interp, final String string) throws TclException {
	...
	static void IllegalType(final Interp interp, final int badType, final int operator) throws TclException {
		throw new TclException(interp, "can't use " + ((badType == ExprValue.DOUBLE) ? "floating-point value" : "non-numeric string")
				+ " as operand of \"" + operatorStrings[operator] + "\"");
	}

	void SyntaxError(final Interp interp) throws TclException {
		throw new TclException(interp, "syntax error in expression \"" + m_expr + "\"");
	}

	void SyntaxError(final Interp interp, final String whyMsg) throws TclException {
		throw new TclException(interp, "syntax error in expression \"" + m_expr + "\": " + whyMsg);
	}

	static void DivideByZero(final Interp interp) throws TclException {
		interp.setErrorCode(TclString.newInstance("ARITH DIVZERO {divide by zero}"));
		throw new TclException(interp, "divide by zero");
	}

	public static void IntegerTooLarge(final Interp interp) throws TclException {
	...
	static void DoubleTooLarge(final Interp interp) throws TclException {
	...
	static void DoubleTooSmall(final Interp interp) throws TclException {
	...
	static void DomainError(final Interp interp) throws TclException {
	...
	static void EmptyStringOperandError(final Interp interp, final int operator) throws TclException {
	...
	public static void ExprParseObject(final Interp interp, final TclObject obj, final ExprValue value) throws TclException {
		...
		if (obj.isIntType()) {
			...
			// Inline TclDouble.get()
					((TclDouble) obj.getInternalRep()).value, (obj.hasNoStringRep() ? null : obj.toString()));
			return;
		}
	...
	public static void ExprParseString(final Interp interp, final TclObject obj, final ExprValue value) {
		char c;
		long ival;
		double dval;
	 ...
	 * @return the value of the expression.
	 */
	private ExprValue ExprGetValue(final Interp interp, final int prec) throws TclException {
		int operator;
		...
		if (!gotOp) {// SETT breakpoint
			value2 = ExprLex(interp);
		}

		while (true) {
			operator = m_token;
			if ((operator < MULT) || (operator >= UNARY_MINUS && operator < STRSTARTSWITH)) {
	...
	public static void evalUnaryOperator(final Interp interp, final int operator, final ExprValue value) throws TclException {
		switch (operator) {
		case UNARY_MINUS:
			if (value.isIntType()) {
		...
		case NOT:
			if (value.isIntType()) {
				...
				if (tok != null && tok.length() == s_len) {
					if ("true".startsWith(tok) || "on".startsWith(tok) || "yes".startsWith(tok)) {
						value.setIntValue(0);
					} else {
						value.setIntValue(1);
					}
	...
	public static void evalBinaryOperator(final Interp interp, final int operator, final ExprValue value, // value
																											// on
																											// left
																											// hand
																											// side
		...
		case MOD:
			...
			if ((neg_divisor && (remainder > 0)) || (!neg_divisor && (remainder < 0))) {
				remainder = -remainder;
			}
	 ...
	 */
	ExprValue mathFunction(final Interp interp) throws TclException {
		int startIdx = m_ind;
		ExprValue value;
		String funcName;
		...
		for (; m_ind < m_len; m_ind++) {
			if (!(Character.isLetterOrDigit(m_expr.charAt(m_ind)) || m_expr.charAt(m_ind) == '_')) {
				break;
			}
		}
		...
		mathFunc = mathFuncTable.get(funcName);
	...
	public void evalMathFunction(final Interp interp, final String funcName, final ExprValue[] values, final boolean releaseValues,
			final ExprValue result) throws TclException {
		MathFunction mathFunc = mathFuncTable.get(funcName);
		if (mathFunc == null) {
			throw new TclException(interp, "unknown math function \"" + funcName + "\"");
		}
	...
	void evalMathFunction(final Interp interp, final String funcName, final MathFunction mathFunc, final ExprValue[] values,
			final boolean releaseValues, final ExprValue result) throws TclException {
	...
	void registerMathFunction(final String name, final MathFunction mathFunc) {
	...
	public static boolean looksLikeInt(final String s, final int len, int i, final boolean whole) {
		char c = '\0';
	...
	static void checkIntegerRange(final Interp interp, final double d) throws TclException {
		if (d < 0) {
			if (d < TCL.INT_MIN) {
	...
	static void checkDoubleRange(final Interp interp, final double d) throws TclException {
	...
	static String getBooleanToken(final String tok) {
	...
	public final void releaseExprValue(final ExprValue val) {
		...
		if (false) {
			...
			for (int i = cachedExprIndex; i < cachedExprLength; i++) {
				if (cachedExprValue[i] != null && val == cachedExprValue[i]) {
					throw new TclRuntimeError("released ExprValue is already in cache slot " + i);
				}
			}
		}
...
abstract class NoArgMathFunction extends MathFunction {
	...
	@Override
	void apply(final Interp interp, final ExprValue[] values) throws TclException {
		throw new TclRuntimeError("NoArgMathFunction must be invoked via apply(interp, ExprValue)");
	}
}
...
class RandFunction extends NoArgMathFunction {
	...
	@Override
	void apply(final Interp interp, final ExprValue value) throws TclException {
		statApply(interp, value);
	}

	static void statApply(final Interp interp, final ExprValue value) throws TclException {
